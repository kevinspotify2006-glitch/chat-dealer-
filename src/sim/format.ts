export const int = new Intl.NumberFormat('en-GB', { maximumFractionDigits: 0 });
...
export function yearOf(day: number): number {
  return calendar(day).year;
}

/** Short date for tables: "12 Mar". */
export function shortDate(day: number): string {
  const c = calendar(day);
  return `${c.dayOfMonth} ${c.monthName.slice(0, 3)}`;
}

export function km(value: number): string {
  return `${int.format(Math.round(value))} km`;
}
