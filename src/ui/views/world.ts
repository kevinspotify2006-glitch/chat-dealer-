  ...
  const onDown = (e: PointerEvent): void => {
    ...
    if (ui.build && e.button === 0) {
      if (t.kind === 'move' && t.dragging && e.pointerType === 'mouse') {
        ...
        if (obj && OBJ_BY_ID[loc.lot.objects.find((o) => o.id === obj)?.defId ?? '']) { ui.context = contextForZone(zoneAt(loc.lot, Math.floor(w.x), Math.floor(w.y))); gesture = 'pressObj'; pressId = obj; pressAt = w; pickReturn = true; }
