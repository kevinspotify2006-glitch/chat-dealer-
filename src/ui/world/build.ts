...
export function buildPalette(host: BuildHost): HTMLElement {
  ...
  for(const area of BUILD_CONTEXTS) contextBar.appendChild(h('button',{class:'bp-context'+(area.id===ctxId?' active':''),title:area.subtitle,aria:{label:area.name+': '+area.subtitle,pressed:String(area.id===ctxId)},on:{click:()=>{haptic(6);host.setContext(area.id);}}},icon(area.icon,17),h('span',{text:area.name})));
  root.appendChild(h('div',{class:'bp-context-head'},h('div',{class:'bp-context-title'},h('strong',{text:ctxDef.name}),h('span',{text:ctxDef.subtitle}),h('button',{class:'bp-context-rooms',title:'Rooms & areas',on:{click:()=>{haptic(6);host.setCategory('zones');}}},icon('garage',14),h('span',{text:'Rooms'}))),contextBar));
  const current = (LEGACY_TAB[cat] ?? cat) as BuildTabId;
  const visibleTabs=new Set<BuildTabId>(ctxDef.tabs); visibleTabs.add('structure');
