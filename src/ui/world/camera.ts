...
export class Camera {
  ...
  toWorld(sx: number, sy: number): { x: number; y: number } {
    ...
    const b = this.basis(), sxn = sx - this.width / 2, syn = sy - this.height / 2;
  ...
  panBy(dxPx: number, dyPx: number): void {
    if (this.projection === 'topdown') {
      this.x -= dxPx / this.zoom; this.y -= dyPx / this.zoom;
    } else {
      const b = this.basis(), det = b.ax * b.by - b.bx * b.ay;
      this.x -= (dxPx * b.by - b.bx * dyPx) / det;
      this.y -= (b.ax * dyPx - dxPx * b.ay) / det;
    }
    this.clamp();
  }
  ...
  fit(x0: number, y0: number, x1: number, y1: number, pad = { top: 60, bottom: 90, left: 20, right: 20 }): void {
    ...
    const worldW = Math.max(1, x1 - x0), worldH = Math.max(1, y1 - y0);
    if (this.projection === 'topdown') this.zoom = Math.max(this.min, Math.min(this.max, Math.min(w / worldW, h / worldH)));
    else this.zoom = Math.max(this.min, Math.min(this.max, Math.min(w / ((worldW + worldH) * 0.84), h / ((worldW + worldH) * 0.42))));
    this.x = (x0 + x1) / 2 - (pad.left - pad.right) / 2 / this.zoom;
    this.y = (y0 + y1) / 2 - (pad.top - pad.bottom) / 2 / this.zoom;
    this.clamp();
  }
  ...
  clamp(): void {
    const b = this.bounds, halfW = this.width / 2 / this.zoom, halfH = this.height / 2 / this.zoom;
